...
const SQUIRCLE_32 = 'M16 0c12.357 0 16 3.643 16 16s-3.643 16-16 16S0 28.357 0 16 3.643 0 16 0Z'
const SQUIRCLE_24 = 'M12 0c9.268 0 12 2.732 12 12s-2.732 12-12 12S0 21.268 0 12 2.732 0 12 0Z'
...
function baselineY(lineBoxTop: number, lineHeight: number, fontSize: number): number {
...
function generateBadge(
  ...
  const largeY = baselineY(textBlockTop + SMALL_LINE_HEIGHT, LARGE_LINE_HEIGHT, LARGE_FONT_SIZE)
  ...
  const largePath = textToPath(largeText, TEXT_X, largeY, LARGE_FONT_SIZE, LARGE_LETTER_SPACING)

  const largeW = textWidth(largeText, LARGE_FONT_SIZE, LARGE_LETTER_SPACING)
...
function resolveBadgeIcon(
  ...
  if (variant === 'dark') {
    ...
    if (existsSync(darkPath)) return { svg: readFileSync(darkPath, 'utf-8'), isBadge: true }
  }
  const badgePath = join(dir, 'badge.svg')
  if (existsSync(badgePath)) return { svg: readFileSync(badgePath, 'utf-8'), isBadge: true }
