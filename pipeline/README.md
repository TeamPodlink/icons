...
| `build-registry.mjs` | any | Emits the shadcn registry (`apps/web/public/r/*.json`); `SITE_URL` env sets absolute URLs; asset URLs pin the immutable R2 release prefix `https://assets.icons.podlink.com/<version>` (version from `packages/refraction/package.json`); `RELEASE_CHECK=1` HEADs the release manifest and fails if the prefix was never uploaded |
...
The web app's `predev`/`prebuild` run data + sync + registry + api
